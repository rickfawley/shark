...
# ---------------------------------------------------------------- #
# Lambda function to enable scaling  - in this case, by range
# ---------------------------------------------------------------- #

scaler = lambda X: (X - X.mean(axis=0)) / (X.max(axis=0) - X.min(axis=0))
...
#    y    = np.loadtxt(labelsFile, delimiter=',')
    ...
    experiment = fetch_openml(name='glass', version=1, as_frame=True)
    data       = scaler(experiment.data)
    y          = experiment.target
